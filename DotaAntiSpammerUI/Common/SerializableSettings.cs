...
namespace DotaAntiSpammerNet.Common
{
    public class SerializableSettings<T> : ISettings<T> where T : new()
    {
        ...
        {
            get => _file ?? (_file = RelativeFile.XmlFile(typeof(T).Name, "Settings"));
            set => _file = value;
        }
        ...
        public string Path
        {
            get
            {
                if (string.IsNullOrEmpty(_path)) _path = SettingsFile.GetFullPath();
        ...
        public T Current
        {
            get
            {
                if (_currentExist) return _current;
                _current = new T();
                _currentExist = true;
        ...
        public void Save(string file = "")
        {
            ...
            if (File.Exists(file) && BackupOnSave) BackupSettings(file);
        ...
        {
            if (string.IsNullOrEmpty(location)) location = Path;

            if (!File.Exists(location)) Save(location);
        ...
        protected void BackupSettings(string target = "")
        {
            ...
            if (extension == null) throw new ArgumentNullException(Path + " does not contain an extension.");
